# 🇮🇹 Viaje a Italia 2026

Planeación del viaje de Vicente + acompañante a Italia, **25 sep → 12 oct 2026**
(16 noches). Repositorio usado como workspace vivo: vuelos, hospedajes, itinerario,
presupuesto y pendientes.

> Para el contexto completo y las convenciones de trabajo, ver [`CLAUDE.md`](./CLAUDE.md).

## Resumen

| | |
| --- | --- |
| **Viajeros** | 2 adultos |
| **Ruta** | Cancún (CUN) ⇄ Milán Malpensa (MXP) vía Frankfurt |
| **Fechas** | vie 25 sep → lun 12 oct 2026 |
| **Aerolínea** | Condor (Economy) · Booking #16267799 · ~$17,888 MXN ✅ |
| **Noches en tierra** | 16 |

## Itinerario en una línea

Milán → Como → Bérgamo → Verona → Venecia → Padua → Bérgamo → ✈️ **París** → ✈️ Roma → Florencia → Pisa → **Volterra** → Milán

## Estado

- ✅ Vuelos internacionales reservados y pagados (Condor)
- ✅ 4 hospedajes reservados (Milán inicio 26–28 sep · Bérgamo 1–2 oct · París 2–5 oct · Volterra 9–11 oct)
- 🔵 Itinerario rediseñado con París (3 noches); sur de Italia fuera
- ⏳ 5 hospedajes por reservar
- ⏳ 2 vuelos low-cost por reservar (Bérgamo→París, París→Roma)
- ⏳ Trenes/buses y entradas a museos por reservar

## Documentación

- [Vuelos](./docs/vuelos.md)
- [Hospedajes](./docs/hospedajes.md)
- [Ciudades](./docs/ciudades.md)
- [Itinerario](./docs/itinerario.md)
- [Transporte](./docs/transporte.md)
- [Presupuesto](./docs/presupuesto.md)
- [Pendientes](./docs/pendientes.md)

## Herramientas

- [`tools/`](./tools/README.md) — consultar precios/horarios de Omio en vivo desde una sesión local (conecta a tu Chrome vía CDP).
